...
import 'package:flutter_taxi_user/Components/autoRotationMarker.dart' as rm;
import 'package:flutter_taxi_user/Components/loading.dart';
import 'package:flutter_taxi_user/Screen/Directions/screens/chat_screen/chat_screen.dart';
import 'package:flutter_taxi_user/Screen/Directions/widgets/arriving_detail_widget.dart';
import 'package:flutter_taxi_user/Screen/Directions/widgets/booking_detail_widget.dart';
import 'package:flutter_taxi_user/app_router.dart';
import 'package:flutter_taxi_user/data/Model/direction_model.dart';
...
class _DirectionsViewState extends State<DirectionsView> {
  ...
  BitmapDescriptor ic_dropoff_48,
      ic_dropoff_96,
      ic_pick_48,
      ic_pick_96,
      icon_car_32,
  ...
  void initState() {
    ...
    BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)),
            'assets/image/marker/ic_dropoff_48.png')
        .then((onValue) {
      ic_dropoff_48 = onValue;
    });
    BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)),
            'assets/image/marker/ic_dropoff_96.png')
        .then((onValue) {
      ic_dropoff_96 = onValue;
    });
    BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)),
    ...
    BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)),
            'assets/image/marker/icon_car_32.png')
        .then((onValue) {
      icon_car_32 = onValue;
    });
    BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(48, 48)),
            'assets/image/marker/icon_car_120.png')
        .then((onValue) {
      icon_car_120 = onValue;
    });
  }
  ...
  addMakers() async {
    ...
    final Marker marker = Marker(
      ...
      icon: checkPlatform ? ic_dropoff_48 : ic_dropoff_96,
      onTap: () {},
    );
    ...
    final Marker markerTo = Marker(
      ...
      icon: checkPlatform ? ic_pick_48 : ic_pick_96,
      onTap: () {},
    );
  ...
  addMakersDriver(LatLng _position) async {
    final MarkerId markerDriver = MarkerId("driver");
    final Marker marker = Marker(
      markerId: markerDriver,
      position: _position,
      icon: checkPlatform ? icon_car_32 : icon_car_120,
      draggable: false,
      rotation: 0.0,
  ...
  handSubmit() {
    ...
    Map<String, dynamic> data = {
      "company": "UKRAINE",
      "id": userinfo.ids,
      "destination": widget?.placeBloc?.locationSelect.name,
      "distance": distance,
      "km": distance,
      "latitude": 50.447730,
      "longitude": 30.542720,
      "notification": "string",
      "price": 0,
      "source": widget?.placeBloc?.formLocation.name,
      "type": "taxi4",
    ...
    ApiHelper.postRequest1(Constants.BASE_URL_SEARCH_IMAGE, data).then((data) {
      cdriverdata = jsonDecode(data.data);
      if (!data.status) {
      } else {
        print(data.message);
        if (data.status) {}
                  ...
                  : isResult == true
                      ...
                      : BookingDetailWidget(
                          ...
                          onTapOptionMenu: () => showDialog(
                              context: context, child: dialogOption()),
                          onTapPromoMenu: () => showDialog(
                              context: context, child: dialogPromoCode()),
                        ),
            ),
          ),
        ],
